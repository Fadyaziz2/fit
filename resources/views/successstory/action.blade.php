...
<div class="d-flex align-items-center">
    @if($auth_user && ($auth_user->hasRole('admin') || $auth_user->can('successstory-edit') || $auth_user->can('successstory')))
        <a class="btn btn-sm btn-icon btn-success me-2" href="{{ route('successstory.edit', $id) }}" data-bs-toggle="tooltip"
           title="{{ __('message.update_form_title',['form' => __('message.successstory') ]) }}">
    ...
    @endif
    @if($auth_user && ($auth_user->hasRole('admin') || $auth_user->can('successstory-delete') || $auth_user->can('successstory')))
        {{ html()->form('POST', route('successstory.destroy', $id))->attribute('data--submit', 'successstory-delete'.$id)->open() }}
        {{ html()->hidden('_method', 'DELETE') }}
            <a class="btn btn-sm btn-icon btn-danger" href="javascript:void(0)" data-bs-toggle="tooltip" data--submit="successstory-delete{{$id}}"
               data--confirmation="true" data-title="{{ __('message.delete_form_title',['form' => __('message.successstory') ]) }}"
               data-message="{{ __('message.delete_msg') }}"
               title="{{ __('message.delete_form_title',['form' => __('message.successstory') ]) }}">
                <span class="btn-inner">
                    <svg width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
