...
<div class="d-flex align-items-center">
    @if($auth_user && ($auth_user->hasRole('admin') || $auth_user->can('discount-code-edit') || $auth_user->can('discount-code')))
        <a class="btn btn-sm btn-icon btn-success me-2" href="{{ route('discount-codes.edit', $id) }}" data-bs-toggle="tooltip"
           title="{{ __('message.update_form_title',['form' => __('message.discount_code') ]) }}">
    ...
    @endif
    @if($auth_user && ($auth_user->hasRole('admin') || $auth_user->can('discount-code-delete') || $auth_user->can('discount-code')))
        {{ html()->form('POST', route('discount-codes.destroy', $id))->attribute('data--submit', 'discount-code-delete'.$id)->open() }}
        {{ html()->hidden('_method', 'DELETE') }}
            <a class="btn btn-sm btn-icon btn-danger" href="javascript:void(0)" data-bs-toggle="tooltip" data--submit="discount-code-delete{{$id}}"
               data--confirmation="true" data-title="{{ __('message.delete_form_title',['form' => __('message.discount_code')]) }}"
               data-message="{{ __('message.delete_msg') }}"
               title="{{ __('message.delete_form_title',['form' => __('message.discount_code') ]) }}">
                <span class="btn-inner">
                    <svg width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
