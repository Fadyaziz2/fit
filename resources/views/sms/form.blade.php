...
<x-app-layout :assets="$assets ?? []">
    <div>
        {{ html()->form('POST', route('sms.send'))->open() }}
        <div class="row">
            <div class="col-lg-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="form-group col-md-12">
                                ...
                                {{ html()->textarea('message')->class('form-control')->rows(4)->attribute('required', 'required')->placeholder(__('message.sms_message_placeholder'))->value(old('message')) }}
                            </div>

                            <div class="form-group col-md-12">
                                {{ html()->label(__('message.sms_recipients') . ' <span class="text-danger">*</span>', 'target')->class('form-control-label d-block') }}
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="target" id="target_all" value="all" {{ old('target', 'all') === 'all' ? 'checked' : '' }}>
                                    <label class="form-check-label" for="target_all">{{ __('message.sms_target_all') }}</label>
                                </div>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="target" id="target_selected" value="selected" {{ old('target') === 'selected' ? 'checked' : '' }}>
                                    <label class="form-check-label" for="target_selected">{{ __('message.sms_target_selected') }}</label>
                                </div>
                            </div>
                            ...
                            <div class="form-group col-md-12 d-none" id="specific-users">
                                ...
                                {{ html()->select('users[]', $users, old('users', []))->class('select2js form-control')->attribute('multiple', 'multiple')->attribute('data-placeholder', __('message.sms_users_placeholder')) }}
                            </div>
                        </div>
                        <hr>
